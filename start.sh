...
php -S 0.0.0.0:$BACKEND_PORT -t backend/public > backend.log 2>&1 &
BACKEND_PID=$!

# Start Frontend Server
echo -e "${YELLOW}Starting Frontend Web Server on 0.0.0.0:$FRONTEND_PORT...${NC}"
php -S 0.0.0.0:$FRONTEND_PORT -t frontend > frontend.log 2>&1 &
FRONTEND_PID=$!
...
echo -e "   To compile and run code, the device on which you are coding"
echo -e "   MUST run the local compilation runner daemon."
echo -e "   Run this command on that specific coding device:"
echo -e "     ${YELLOW}python3 local-runner/runner.py${NC}"
echo -e "   (This binds to port 8000 on that local machine)"
echo -e "${BOLD}${CYAN}=====================================================${NC}"
